...
const OrientationTask13 = () => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 sm:p-6 lg:p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
      <h2 className="text-xl sm:text-2xl font-bold text-center text-blue-800 dark:text-blue-300 mb-4">แบบทดสอบที่ 13: การรับรู้ - วัน-เวลา ฤดูกาล</h2>
      <p className="text-base sm:text-lg text-center text-gray-700 dark:text-gray-300 mb-6">
      ...
      <div className="space-y-4">
        {orientationItems.map((item, index) => (
          <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
            <label htmlFor={item.id} className="text-base sm:text-lg font-medium text-gray-800 dark:text-gray-200">{item.label}</label>
            <select
              id={item.id}
              value={answers[index]}
              onChange={(e) => handleInputChange(index, e.target.value)}
              className="w-40 sm:w-48 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm sm:text-base dark:bg-gray-600 dark:border-gray-500 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            >
